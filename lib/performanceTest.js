...
PerformanceTest.prototype.logRequestTimes = function (end){
    ...
    if(THIS.count >= THIS.counterCheck){
        ...
        //check if instance of database was sent along
        if(THIS.db){
          THIS.db.getDatabase().insert(log, function (err, newDoc) {
            ...
            if(THIS.callback){
                THIS.callback(
                new Error("Database was not defined. So test result was not saved, but is available as log"),
                {
                  logObject: log,
                  logMessage: logMessage
                });
